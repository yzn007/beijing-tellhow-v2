...
var menu = [{
	...
	handler : function() {
        ...
        if(projectID == '')
//        Ext.MessageBox.alert("方案不能为空，请选择方案。");
            return;
        if(showID =='1'){
        ...
        zoneID  = Ext.getCmp("zoneSelector").code || '';
		...
		document.excelForm.time_id.value = monthID;

        document.excelForm.zone_id.value = zoneID;
...
var monthDS = new Ext.data.JsonStore({
	url : pathUrl + '/bscProject_common.action?method=getProjectMonth',
	root : 'results',
...
var projectStore = new Ext.data.JsonStore({
	url : pathUrl + '/bscProject_common.action?method=getProjectInfo',
	root : 'results',
...
var objDS = new Ext.data.JsonStore({
	url : pathUrl + '/bscProject_common.action?method=getPorjectObjInfo',
	root : 'results',
	totalProperty : 'totalCount',
...
var zoneDS = new Ext.data.JsonStore({
    url : pathUrl + '/bscProject_common.action?method=getZoneInfo',
    root : 'results',
    totalProperty : 'totalCount',
    ...
    listeners : {
        load : function(store, records, options) {
            if (store.getCount() > 0) {
 /*               if (zoneID == ''){
                    zoneID = store.getAt(0).get('zone_id');
                }
                Ext.getCmp("zoneSelector").setValue(zoneID);*/
...
var personRecord = Ext.data.Record.create([ {name: 'link_id'},
    {name: 'link_name'},
    {name: 'id_field'},
    {name: 'label_field'}]);
var data = [['index_type', '指标名称','index_1', '指标名称']];
...
gridSelector = function(obj) {
	...
	var vStore = new Ext.data.JsonStore({
		url : pathUrl + '/bscProject_common.action?method=getProjectIndex',
...
//            m = getMOCmpVal("monthSelector");
            m = Ext.getCmp("monthSelector1").getValue();
			p = Ext.getCmp("projectSelector").getValue();
...
//        Ext.MessageBox.alert("方案不能为空，请选择方案。");
	...
	mask.show();
	path = pathUrl + '/bscresult_getResultDhtmlByCondExt.action' + param;
	Ext.Ajax.request({
		url : pathUrl + '/bscresult_getResultDhtmlCountByCondExt.action' + param,
		waitMsg : '正在处理,请稍候......',
		method : 'POST',
		timeout : 30000,
		callback : function(options, success, response) {
