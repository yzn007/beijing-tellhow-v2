...
public class CalculateProcedureOnlyMeasureMain extends Thread implements Procedure{
    ...
    public static void  main(String []args){
        ...
        BscMeasureServiceImpl bscMeasureServiceImpl = (BscMeasureServiceImpl) ioc.getBean("bscMeasureService");
        ...
        calculateProcedureOnlyMeasure.setJdbcManager(new SupportedJdbcManager(template));
...
//        bscMeasureServiceImpl.setBscMeasureDao(bscMeasureDao);
    ...
    public CalculateProcedureOnlyMeasureMain() {
    }
...
//                +"' and measure_id in  (select measure_id from bsc_measure where countperiod = '02')" );
    ...
    protected void parseProjectMeasure(List<IMeasure> projMeasures) throws Exception{
        ...
        for (int i = 0; i < projMeasures.size() && this.run; i++) {
            ...
            System.out.println("正在解析方案指标......("+ (i+1) +"/"+projMeasures.size()+")");
        }
    }
    ...
    protected void executeLowLevelMeasure() throws Exception{
        ...
        for (int i = 0; i < lowMeaCommandList.size() && this.run; i++) {
            ...
            this.status.setIndex(++runStep);
            this.status.updateLogExecutInfo("正在计算底层日指标值......("+ (i+1) +"/"+lowMeaCommandList.size()+")");
            System.out.println("正在计算底层日指标值......("+ (i+1) +"/"+lowMeaCommandList.size()+")");
        }
        //月
        dateFrm = this.date.substring(0,7);
        ...
        for (int i = 0; i < lowMeaCommandList.size() && this.run; i++) {
            ...
            this.status.setIndex(++runStep);
            this.status.updateLogExecutInfo("正在计算底层月指标值......("+ (i+1) +"/"+lowMeaCommandList.size()+")");
            System.out.println("正在计算底层月指标值......("+ (i+1) +"/"+lowMeaCommandList.size()+")");
        }
        //季
        dateFrm = getSeasonString(this.date);
        sql = "select * from " + this.commandTable + " where date='"+dateFrm+"' order by exe_order_id";
        ...
        for (int i = 0; i < lowMeaCommandList.size() && this.run; i++) {
            ...
            this.status.setIndex(++runStep);
            this.status.updateLogExecutInfo("正在计算底层季指标值......("+ (i+1) +"/"+lowMeaCommandList.size()+")");
            System.out.println("正在计算底层季指标值......("+ (i+1) +"/"+lowMeaCommandList.size()+")");
        }
        //年
        dateFrm = this.date.substring(0,4);
        sql = "select * from " + this.commandTable + " where date='"+dateFrm+"' order by exe_order_id";
