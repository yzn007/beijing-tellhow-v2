...
public class CalculateProcedureOnlyMeasureMain extends Thread implements Procedure{
    public static void  main(String []args){
        ...
        SupportedJdbcManager supportedJdbcManager = new SupportedJdbcManager(template);
...
//        supportedJdbcManager = new SupportedJdbcManager(template);
                calculateProcedureOnlyMeasure.setJdbcManager(supportedJdbcManager);
...
//            List<Map<String, Object>> projMeasures = this.measureService.getProjectMeasure(this.projectID);
            ...
            System.out.println("正在解析底层指标计算公式......"+start);
...
//                +" and measure_id in  (select measure_id from bsc_measure where countperiod = '02')" );
    ...
    protected void parseLowLevelMeasure(List<IMeasure> measureList) throws Exception{
        ...
        for (int i = 0; i < measureList.size() && this.run; i++) {
            ...
            this.status.setIndex(++runStep);
            if(start==0)
            this.status.updateLogExecutInfo("正在解析底层指标计算公式......("+ (i+1) +"/"+measureList.size()+")"+start);
